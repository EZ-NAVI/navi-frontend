  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const data = initialList.length > 0
          ? initialList
          : await fetchReportsByCluster(String(clusterId), token ?? undefined);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          ...
          if (rid) {
            try { navigation.navigate('SafeRoute', { openClusterModal: true, openClusterId: String(clusterId) }); } catch (e) {}
            try { onClose(); } catch (e) {}
          ...
          <View style={styles.cardHeaderRow}>
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                if (!tokenCheck) {
                  setAlertTitle("안내");
                  setAlertMsg("체험해보기 상태에서는 '이제 없어요' 기능을 사용할 수 없어요!");
                  setAlertHideCancel(true);
                  setAlertConfirm(null);
                  setAlertVisible(true);
                  return;
          ...
          <View style={styles.cardFooter}>
            <View style={{ flex: 1 }}>
              ...
              {(() => {
                ...
                const toShow = commentsArr.slice(0, 3).map((c: any) =>
                  typeof c === 'string' ? c :
                    c.content ?? c.text ?? c.comment ?? c.body ?? JSON.stringify(c)
                );
                return (
                  <View>
              ...
              <View style={styles.emojisRow}>
                  ...
                  if (role === 'parent') {
                    return (
                      <View style={[styles.emojiBtn, { opacity: 1, marginRight: 6 }]}>
                        <Image source={require('../asset/emoji_good.png')} style={{ width: 28, height: 28 }} />
                  ...
                  if (role === 'parent') {
                    return (
                      <View style={[styles.emojiBtn, { opacity: 1 }]}>
                        <Image source={require('../asset/emoji_soso.png')} style={{ width: 28, height: 28 }} />
                ...
                {(() => {
                  ...
                  if (role === 'parent') {
                    return (
                      <View style={[styles.emojiBtn, { opacity: 1 }]}>
                        <Image source={require('../asset/emoji_bad.png')} style={{ width: 28, height: 28 }} />
        ...
        <View style={styles.tabsRow}>
          {categories.map((c) => (
            <TouchableOpacity
              ...
              style={[styles.tabBtn, selectedCategory === c ? styles.tabBtnActive : null]}
            >
...
const styles = StyleSheet.create({
  ...
  tabBtn: { marginRight: 8, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: '#eee' },
